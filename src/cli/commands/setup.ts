...
function setupInstructionBlock(profile: AgentProfile, installRoot: string, mode: SetupMode): string {
  ...
  if (mode === "minimal") {
    return [
      ...
      "- Claude Code: match native skills first; Codex, Gemini, and generic agents should start with `apx discover`.",
      "- Use `apx discover` and `apx inventory` for uncertain matches, non-skill assets, staged assets, and installed-only assets.",
      "- Use `apx` commands to inspect, validate, and extend setup. Do not manually edit config files without running `apx` first.",
  ...
  if (mode === "recommended") {
    return [
      ...
      "- Use `apx discover` and `apx inventory` for uncertain matches, non-skill assets, staged assets, and installed-only assets.",
      "- Use `apx` commands to inspect, validate, and extend setup. Do not manually edit config files without running `apx` first.",
      "- Skills are at `agent-powerups/skills/`. Plugins are at `agent-powerups/plugins/`.",
