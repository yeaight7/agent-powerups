...
class ClaudeAcpClient {
  ...
  constructor(
    ...
    child.on("exit", (code, signal) => {
      const error = new Error(`Claude ACP exited: ${signal ?? code ?? "unknown"}`);
  ...
  async newSession(model?: string): Promise<string> {
    ...
    if (!sessionId || typeof sessionId !== "string") {
      throw new Error("Claude ACP did not return a sessionId");
    }
    if (model) {
  ...
  private request(method: string, params: unknown, timeoutMs = 30000): Promise<AcpMessage> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Timed out waiting for Claude ACP method: ${method}`));
      }, timeoutMs);
      this.pending.set(id, {
        resolve: (response) => {
          clearTimeout(timer);
...
export async function buildClaudeRelayBackend(
  repoRoot: string,
  model: string | undefined,
  log: (line: string) => void,
): Promise<{ backend: RelayBackend; acpSessionId: string }> {
  const commandPath = await resolveCommand("npx");
  if (!commandPath) {
    throw new Error("npx required to run Claude ACP adapter.");
  }

  const args = ["@zed-industries/claude-agent-acp"];
  const acpChild =
  ...
  const acp = new ClaudeAcpClient(acpChild, repoRoot, log);
